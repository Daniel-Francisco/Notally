...
class MakeContacts : NotallyActivity() {
   ...
   override fun getLabelGroup() = binding.LabelGroup
    ...
    private fun setupListeners() {
       binding.EnterTitle.addTextChangedListener(onTextChanged = { text, start, count, after ->
        ...
        binding.contactname.addTextChangedListener(onTextChanged = { text, start, count, after ->
            model.name = text.toString().trim()
        })
        binding.phonenumber.addTextChangedListener(onTextChanged = { text, start, count, after ->
            model.phoneNumber = text.toString().trim()
        })

        binding.email.addTextChangedListener(onTextChanged = { text, start, count, after ->
            model.email = text.toString().trim()
        })
    }
    ...
    private fun setStateFromModel() {
       val formatter = BaseNoteModel.getDateFormatter(getLocale())
        ...
        binding.contactname.setText(model.name)
